...
/// FluentValidation 12 removed the TestHelper ShouldHaveValidationErrorFor API.
/// We use ValidateAsync directly and assert on the returned ValidationResult,
/// which is equally explicit and not tied to a deprecated API.
/// </summary>
public sealed class RegisterUserValidatorTests
{
    ...
    {
        var result = await _validator.ValidateAsync(
            ValidCommand() with { Email = new string('a', 249) + "@b.com" }); // 255 chars — exceeds 254 limit
    ...
    public async Task Phone_WhenNull_HasNoErrors()
    {
        ...
        result.Errors.Should().NotContain(e =>
            e.PropertyName == nameof(RegisterUserCommand.Phone));
    }
    ...
    [Fact]
    public async Task Password_WithNoSpecialCharacter_HasSpecialCharError()
    {
        var result = await _validator.ValidateAsync(ValidCommand() with { Password = "NoSpecial1" });
    ...
    [InlineData("A1!aaaaa")]   // exactly 8 chars — boundary value
